...
def plotSingleFile(filename):
	...
	pl.ylabel('Penalty')
	pl.legend(loc=1)
	#the third subplot plots the (left out) best penalized cost and the best feasible cost
	pl.subplot(413)
	#pl.plot(data[:,0], data[:,6], label=headerArray[6])
	pl.plot(data[:,0], dontPlotZero(data[:,7]), label=headerArray[7])
	pl.xlabel('Iteration')
	pl.ylabel('Cost')
	axes = pl.gca()
	ylim = axes.get_ylim()
	...
	#the fourth subplot plots the average violations of the population
	pl.subplot(414);
	...
	'''
	#the fifth subplot plots the violations of the individual with the best penalized cost
	pl.subplot(614);
	...
	#the sixth subplot plots the average number of visits per voyage in the best feasible solution and the average of the population
	pl.subplot(616);
	...
	the seventh subplot plots the gap from the best feasible solution to the best known solution
	pl.subplot(717);
